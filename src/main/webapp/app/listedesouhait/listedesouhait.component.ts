  ...
  supprimerVoiture2(id: number | undefined): void {
    ...
    if (id != undefined) {
      this.souhaitService.supprimerVoitureSouhait(this.username, id).subscribe((res: boolean) => {
        ...
        if (res) {
          this.getSouhait();
          this.dialog.open(HintComponent, { data: 'Retiré de vos favoris !', position: { left: '39rem' } });
    ...
    if (this.voitureChoisit.id != null && this.voitureChoisit.version != null) {
      this.panierService
        ...
        .subscribe((res: boolean) => {
          ...
          if (res === true) {
            this.dialog.open(HintComponent, { data: 'Article déplacé vers le panier !', position: { left: '39rem' } });
          }
